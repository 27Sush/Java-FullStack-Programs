...
public class WeatherService {
	...
	// to fetch all weather reports () from DB
	public List<Weather> getAllWeather() {
		return repository.findAll();
	}
	
	// to fetch weather report (specific obj) based on PrimaryKey
	...
	public String updateWeather(int id, String newCondt, String newTemp) {
		...
		if(w!=null) {
			...
			return "Weather report doesn't exist...";
		}
	}
	
	// to delete weather report
	...
	// to find weather report by city
	public List<Weather> getByCity(String city){
		return repository.findByCity(city);
	}

	
	public String updateByDate(String cond, String date) {
		int res = repository.updateWeatherByDate(cond, date);
		if(res==0) {
